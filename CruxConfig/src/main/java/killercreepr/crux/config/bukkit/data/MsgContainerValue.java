...
public class MsgContainerValue extends ConfigValue<MsgContainer> {
    ...
    public @Nullable MsgContainer get(@NotNull CruxConfig cfg, @NotNull String path) {
        ...
        CreateTitle title = new killercreepr.crux.config.bukkit.data.CreateTitleValue().get(cfg, addDot(path) + "title");
        CreateSound sound = new killercreepr.crux.config.bukkit.data.CreateSoundValue().get(cfg, addDot(path) + "sound");
