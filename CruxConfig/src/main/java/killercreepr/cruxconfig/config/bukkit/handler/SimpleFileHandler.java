...
public abstract class SimpleFileHandler<T> extends FileHandler<T>{
    ...
    public abstract @NotNull FileElement serializeToFile(@NotNull T object);
    public abstract @Nullable T deserializeFromFile(@NotNull FileElement e);

    @Override
    public @Nullable T deserializeFromYaml(@NotNull YamlContext context, @Nullable YamlElement e) {
        if(e==null) return null;
        return deserializeFromFile(e);
    }

    @Override
    public @Nullable T deserializeFromJson(@NotNull JsonContext context, @Nullable JsonElement e) {
        if(e==null) return null;
        return deserializeFromFile(FileElement.fromJson(e));
    }
}
