...
public class FileBlockTag extends SimpleFileHandler<BlockTag> {
    @Override
    public @NotNull FileElement serializeToFile(@NotNull FileContext<?> context, @NotNull BlockTag object) {
    ...
    public @Nullable BlockTag deserializeFromFile(@NotNull FileContext<?> ctx, @NotNull FileElement e, @NotNull Key key){
        if(!(e instanceof FileObject o)) return null;
        FileElement values = o.get("values");
        if(values==null) return null;
        return deserializeValues(ctx, values, key);
    }
    public @Nullable BlockTag deserializeValues(@NotNull FileContext<?> ctx, @NotNull FileElement e, @NotNull Key key) {
        if(e instanceof FileGeneric single){
            String itemKey = single.getAsString();
            if(itemKey.startsWith("#")){
