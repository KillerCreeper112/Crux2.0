...
public class SimpleShapeCachedScheduler implements ShapeScheduler {
    ...
    public SimpleShapeCachedScheduler(@NotNull CreateShape shape, @NotNull Consumer<ShapeTickLocationContext> consumer, @NotNull Consumer<ShapeTickContext> tickConsumer, @Nullable Runnable endTask) {
        this.shape = shape;
    ...
    public CruxScheduledTask schedule(int totalTicksTime) {
        BukkitRunnable runnable = new ScheduledShapeCacheRunnableLocation(
            shape.generateCache(), consumer, tickConsumer, endTask, totalTicksTime
        );
