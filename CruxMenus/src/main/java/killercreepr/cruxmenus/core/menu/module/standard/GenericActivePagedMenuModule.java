...
public abstract class GenericActivePagedMenuModule<T> extends SimpleActiveMenuModuled implements IActivePagedMenuModule<T> {
    ...
    public GenericActivePagedMenuModule(@NotNull String id,
                                        @NotNull MenuModule module,
                                        @NotNull NumberProvider indexes, @Nullable Predicate<T> valueFilter,
    ...
    public boolean canShow(T value){
        return valueFilter == null || valueFilter.test(value);
    }

    public @NotNull List<T> filterValues(@NotNull List<T> value, @NotNull TextParserContext ctx){
        if(valueFilter == null) return value;
        return value.stream().filter(this::canShow).toList();
    }

    public abstract MergedTagContainer buildTags(T value);

    public GenericActivePagedMenuModule<T> addPage(int amount){
        return setPage(page+amount);
    }

    public GenericActivePagedMenuModule<T> setPage(int amount){
