package killercreepr.cruxpotions.core.command;
...
public class CruxPotionCommands {
    ...
    public static int applyEffectItem(@NotNull CommandSourceStack source, @NotNull Collection<Entity> to,
        ...
        int effected = 0;
        StoredPotion stored = StoredPotion.storedPotion(potion, duration, amplifier);
        for(Entity e : to){
            ...
            List<StoredPotion> storedPotions = crux.getOrDefault(PotionComponents.STORED_CRUX_POTIONS, new ArrayList<>());
            storedPotions.removeIf(s -> s.getPotion().compare(potion));
            storedPotions.add(stored);
            crux.set(PotionComponents.STORED_CRUX_POTIONS, storedPotions);
    ...
    public static int applyEffect(@NotNull CommandSourceStack source, @NotNull Collection<Entity> to,
        ...
        int effected = 0;
        for(Entity e : to){
            SimplePotionHolder holder = EntityMemory.getOrCreateDataHolder(e, SimplePotionHolder.class);
    ...
    public static int clearEffectsItem(@NotNull CommandSourceStack source, @NotNull Collection<Entity> from){
        ...
        for(Entity e : from){
            ...
            CruxItem.wrap(item).set(PotionComponents.STORED_CRUX_POTIONS, null);
            Crux.handlers().item().update(item, e);
            if(!(e instanceof Player)){
                le.getEquipment().setItemInMainHand(item, true);
            }
    ...
    public static int removeEffectItem(@NotNull CommandSourceStack source, @NotNull Collection<Entity> from,
        ...
        for(Entity e : from){
            ...
            List<StoredPotion> storedPotions = crux.get(PotionComponents.STORED_CRUX_POTIONS);
            if(storedPotions==null) continue;
            storedPotions.removeIf(stored -> stored.getPotion().compare(potion));
            crux.set(PotionComponents.STORED_CRUX_POTIONS, storedPotions.isEmpty() ? null : storedPotions);
            Crux.handlers().item().update(item, e);
            if(!(e instanceof Player)){
                le.getEquipment().setItemInMainHand(item, true);
            }
